...
def golden_age
  ...
  Movie.select('yr / 10 * 10 AS decade')
       .group('decade').order('AVG(score) DESC').limit(1)
       .first.decade
...
def costars(name)
  ...
  Actor.joins(:castings).where(castings: { movie_id: subquery })
...
def starring(whazzername)
  ...
  Movie.joins(:actors).where('LOWER(actors.name) LIKE LOWER(?)', matcher)
end
