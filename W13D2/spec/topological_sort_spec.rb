...
describe 'TopologicalSort' do
  ...
  it "sorts the vertices" do
    ...
    Edge.new(v7, v8)
    10.times do
      # rand_vertices = [v6, v3, v5, v4, v1, v8, v7, v2]
      rand_vertices = vertices.shuffle
      # puts '============'
      # rand_vertices.each { |v| puts v.value }
      # puts '============'
      expect(solutions).to include(topological_sort(rand_vertices).map{ |vert| vert.value})
    end
  end
